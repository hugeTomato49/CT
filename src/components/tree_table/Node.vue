<template>
    <circle 
        v-if="nodeData.leaf"
        :cx="(nodeData.level - 1) * columnWidth + columnWidth/2" 
    ...
    viewBox="0 0 256 512"
    :transform="`translate(${(nodeData.level - 1) * columnWidth + columnWidth/2 - 128 * scaleFactor}, ${index * rowHeight + rowHeight / 2 - 256 * scaleFactor}) scale(${scaleFactor})`"
    ...
    v-if="!foldState && !nodeData.leaf"
    viewBox="0 0 320 512"
    :transform="`translate(${(nodeData.level - 1) * columnWidth + columnWidth/2 - 160 * scaleFactor}, ${index * rowHeight + rowHeight / 2 - 256 * scaleFactor}) scale(${scaleFactor})`"
